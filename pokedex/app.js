...
const getOnlyFulfilled = async ({arr, func}) => {
  ...
  return responses.filter(res => res.status === 'fulfilled')
}
...
const getPokemonsIds = async pokeApiResults => {
  return pokeApiResults.map(({ url }) => {
...
const getPokemons = async () => {
  try {
    ...
    if (!response.ok) { throw Error('Something is happening with me.') }
